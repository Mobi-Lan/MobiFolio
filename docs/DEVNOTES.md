...
- **빌드**: PyInstaller 6.22.3 고정, `npm ci`, 두 실행파일 모두 버전 리소스·아이콘 포함(정식판 `version_info.txt`, 경량판 `version_info_lite.txt`). 코드 서명은 없음(SmartScreen 경고 가능 — 배포 시 zip 의 SHA256 을 릴리스 노트에 적을 것).
- **백신 오탐**: 외부 프로세스(PowerShell 등)를 실행하는 코드는 없다(0.2.2~). PyInstaller 부트로더 직접 컴파일은 **효과가 없다** — 0.2.3 에서 실측 결과 탐지가 5건에서 9건으로 늘었다(처음 보는 바이너리라 평판이 없고, 파이썬 아카이브 구조 탐지는 부트로더와 무관). 0.2.4 부터 기성 부트로더로 되돌림. 남은 수단은 코드 서명(OV 이상)과 릴리스마다 벤더 오탐 제출뿐이다.
...
## 자동 업데이트 (경량판)
설정 → 「업데이트 확인 주소」에 `latest.json` 의 https 주소를 넣으면, 시작할 때(설정으로 끌 수 있음) 그 JSON 만 읽어 새 버전이 있으면 상단에 알림을 띄운다. 「지금 업데이트」를 누르면 새 exe 를 받아 SHA256 을 검증하고, 현재 exe 를 `.bak` 으로 바꾼 뒤 새 파일을 제자리에 두고 새 프로세스를 띄운다(창은 그대로, 페이지가 새 포트로 이동). 새 프로세스가 `.bak` 을 지운다. 앱이 보내는 정보는 없고, 통신은 이 두 요청뿐이다. `latest.json` 은 `release.cmd` 가 만든다 — 형식: `{"version":"0.1.1","url":"https://…/MobiFolioLite-0.1.1.exe","sha256":"…","notes":""}`. `release.cmd` 실행 전에 `set MF_UPDATE_BASE=https://<파일을 올린 주소>` 를 지정하면 url 이 그 주소로 채워진다. Cloudflare 등 정적 호스팅에 `MobiFolioLite-<버전>.exe` 와 `latest.json` 을 같은 폴더에 올리면 된다. **빌드는 배포가 아니다** — `release.cmd` 는 파일만 만들고 사이트는 건드리지 않는다(무엇이 올라갈지 보여만 준다). 어느 판을 내보낼지는 `scripts\publish.cmd` 로 사람이 고른다. 사이트의 `latest.json` 이 바뀌는 순간 설치된 앱들이 그 판으로 자동 업데이트되기 때문이다. 배포는 — `scripts/publish_promo.py` 가 `MobiFolio_promo\site`(배포 저장소 `Mobi-Lan/MobiFolio_WEB` 의 작업 복사본)에 파일을 쓰고, 해시·주소를 검증한 뒤 커밋·푸시하면 Cloudflare Pages 가 <https://fo.mobimml.com> 에 올린다. 직전 한 판은 남겨 둔다(업데이트 중인 사용자가 404 를 받지 않게). 같은 번호로 내용이 다른 exe 를 올리려 하면 스크립트가 막는다 — 이미 받은 사람이 자동 업데이트를 못 타기 때문이다.
