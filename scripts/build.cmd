...
rem Usage: scripts\build.cmd [--nopause]  (runs from the repo root)
chcp 65001 >nul
set PYTHONUTF8=1
cd /d "%~dp0.."
set "NOPAUSE="
...
python -m PyInstaller --noconfirm --clean --onefile --noconsole --name MobiFolioCore --add-data "ui;ui" --version-file packaging\version_info.txt --noupx --icon app\icon\MobiFolio.ico server.py
...
python -m PyInstaller --noconfirm --clean --onefile --noconsole --name MobiFolioLite --add-data "ui;ui" --version-file packaging\version_info_lite.txt --noupx --icon app\icon\MobiFolio.ico server.py
if errorlevel 1 ( echo LITE BUILD FAILED & goto :fail )
rem Mini edition: same backend + mini UI (game-like grid; playlist/play/shuffle/repeat only; 440x920 app window).
python -m PyInstaller --noconfirm --clean --onefile --noconsole --name MobiFolioMini --add-data "mini\ui;ui" --version-file packaging\version_info_mini.txt --noupx --icon app\icon\MobiFolio.ico server.py
if errorlevel 1 ( echo MINI BUILD FAILED & goto :fail )
cd app
