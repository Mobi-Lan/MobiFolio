...
- **백신 오탐 완화** (0.2.2~): 외부 프로세스(PowerShell 등)를 실행하는 코드가 없고, PyInstaller **부트로더를 직접 컴파일**해 쓴다 — 기성 부트로더 바이트 패턴에 붙는 휴리스틱 탐지를 피하기 위해. 방법: VS 2022 Build Tools(C++ 워크로드) 설치 → PyPI 의 `pyinstaller-6.22.3.tar.gz`(sha256 확인) 를 짧은 경로에 풀고 `bootloader\` 에서 vcvars64 환경으로 `python waf all --target-arch=64bit` → 소스 루트에서 `pip install --no-deps --force-reinstall .` (긴 경로에서는 waf 의 컴파일 검사가 실패한다). 이 상태의 site-packages 를 `build.cmd` 의 `pip install pyinstaller==6.22.3` 은 그대로 둔다(같은 버전). 새 PC·재설치 뒤에는 다시 해야 한다. 그래도 ML 기반 판정(Microsoft Wacatac.B!ml 등)은 남을 수 있으니 릴리스마다 VirusTotal 확인 후 오탐 제출.
