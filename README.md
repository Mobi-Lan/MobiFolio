...
## 로컬 API 보안
POST 는 `Host` 가 127.0.0.1/localhost 이고 `X-Requested-With: mobifolio` 헤더가 있을 때만 받는다(브라우저의 다른 사이트가 보내는 폼/스크립트 요청 차단). 본문은 1MB 까지, 객체(JSON object)만. 잘못된 입력은 연결을 끊지 않고 400/500 JSON 으로 답한다.
